...
public class WifiFunction {
    ...
    // 检查当前wifi是否可用
    public int checkWifiWork() {
        return meWifiManager.getWifiState() == meWifiManager.WIFI_STATE_ENABLED ? 1 : 0;
    }
    ...
    public StringBuffer lookUpScan() {
        ...
        for (int i = 0; i < meWifiList.size(); i++) {
            ...
            sb.append((meWifiList.get(i).SSID).toString()).append("\n");
        }
        return sb;
    }

    // 查看配置网络结果
    public StringBuffer lookUpConfigure() {
        StringBuffer sb = new StringBuffer();
        for (int i = 0; i < meWifiConfigurations.size(); i++) {
            sb.append("Index_" + new Integer(i + 1).toString() + ":");
            // 将ScanResult信息转换成一个字符串包
            // 其中把包括：BSSID、SSID、capabilities、frequency、level
            sb.append((meWifiConfigurations.get(i).SSID).toString()).append("\n");
        }
        return sb;
    }
    ...
    public int IsConfiguration(String SSID) {
        for (int i = 0; i < meWifiConfigurations.size(); i++) {
            if (meWifiConfigurations.get(i).SSID.equals("\"" + SSID + "\"") || meWifiConfigurations.get(i).SSID.equals(SSID)) {
                return meWifiConfigurations.get(i).networkId;
            }
        }
        return -1;
    }
    ...
    public String getSSID() {
        return (meWifiInfo == null) ? "NULL" : meWifiInfo.getSSID();
    }
