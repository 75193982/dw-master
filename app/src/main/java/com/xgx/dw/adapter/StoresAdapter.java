...
public class StoresAdapter extends BaseQuickAdapter<SysDept, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder paramBaseViewHolder, SysDept paramStoreBean) {
        paramBaseViewHolder.setText(R.id.title, "编号：" + paramStoreBean.getCountyid() + "   名称：" + paramStoreBean.getFullname()).setText(R.id.subtitle, "地址：" + paramStoreBean.getAddress()).setText(R.id.content, "联系人：" + paramStoreBean.getContact() + "   联系电话：" + paramStoreBean.getTel());
    }
}
